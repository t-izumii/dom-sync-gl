...
- GL を初期化できない環境（初期化の失敗・8 秒のタイムアウト・板の構築中の例外）:
  作りかけの GL を破棄し、`html.no-gl` で DOM と CSS のグラデーションだけで読める構成に倒す
  （文字の板が無いので DOM の文字がそのまま見える）。どの経路でもローダーは必ず先へ進む。
- ローダーの読み上げは `role="status"` の「読み込み中です」「読み込みが完了しました」の 2 回だけ。
  毎フレーム変わるカウンタは `aria-hidden`。ロード中はスキップリンクも出さない。
...
`site/FlipYEffect.ts` をチェーン末尾に足して打ち消している。動きの抑制で流れ（MouseFlow）を
止めるときは反転パスも一緒に止め、有効な pass を 0 にして RT を使わない描画へ戻す。
ライブラリ側で直したら外す。
